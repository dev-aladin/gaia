...
[![codecov](https://codecov.io/gh/aladin/gaia/branch/master/graph/badge.svg)](https://codecov.io/gh/aladin/gaia)
...
`aladinjs` APIs
documented [here](https://github.com/ALADINIO/aladinjs/tree/master/src/storage) and [here](http://aladinjs.github.io/aladinjs/#storage).
...
Instructions on setting up and configuring a Gaia Hub can be found in [this readme](https://github.com/ALADINIO/gaia/blob/master/hub/README.md).
...
Aladin applications use the Gaia storage system to store data on behalf of a user.
...
lookup. Instead, the aladin [authentication flow](https://github.com/ALADINIO/aladinjs/blob/master/src/auth/README.md#authentication-response-payload-schema) provides Alice's chosen application root
...
client-side concerns. Client libraries (such as `aladinjs`) are
...
enabled lookups without traditional DNS (for example, with the Aladin Name
System instead), then gaia hubs could return URLs implementing that scheme. As
the Aladin ecosystem develops and supports these kinds of features, we expect
...
<!--- Aladin Core no longer does this, but Gaia nodes do.  I updated this
...
`.storage` namespace in Aladin, and post her rates per gigabyte in her node's
