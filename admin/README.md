...
To install this service as the `aladin-gaia-admin` program in your `$PATH`:

```
$ sudo npm install -g
$ which aladin-gaia-admin
/usr/bin/aladin-gaia-admin
```
...
command is set to kill all instances of the `aladin-gaia-hub` program, and
...
$ cat /tmp/gaia-admin.json
{
  ...
  "reloadSettings": {
    "command": "/bin/sh",
    "argv": ["-c", "pkill -9 aladin-gaia-hub; nohup /usr/bin/aladin-gaia-hub /tmp/gaia-config.json &"],
    "env": {},
    "setuid": 1000,
    "setgid": 1000
  }
